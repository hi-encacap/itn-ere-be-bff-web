  ...
  imports: [
    NestConfigModule.forRoot({
      ...
      validationSchema: Joi.object({
        ...
        DB_REDIS_HOST: Joi.string().required().default('localhost').allow(''),
        DB_REDIS_PORT: Joi.number().required().default(6379).allow(''),
        DB_REDIS_USERNAME: Joi.string().required().default('encacap').allow(''),
        DB_REDIS_PASSWORD: Joi.string().required().default('').allow(''),
